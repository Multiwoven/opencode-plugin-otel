...
| `OPENCODE_RESOURCE_ATTRIBUTES` | *(unset)* | Comma-separated `key=value` pairs merged into the OTel **resource** (describes the producer). Example: `service.version=1.2.3,deployment.environment=production` |
| `OPENCODE_SPAN_ATTRIBUTES` | *(unset)* | Comma-separated `key=value` pairs added to **spans only** (not logs, metrics, or the resource). Example: `team=platform,deployment.environment=production` |
...
# Tag the OTel resource (producer metadata) with deployment context
export OPENCODE_RESOURCE_ATTRIBUTES="service.version=1.2.3,deployment.environment=production"

# Tag spans only (e.g. high-cardinality dimensions for trace drill-down)
export OPENCODE_SPAN_ATTRIBUTES="team=platform"

# Tag metric data points only (e.g. low-cardinality dashboard dimensions)
export OPENCODE_METRIC_ATTRIBUTES="deployment.environment=production"
```

Each attribute channel targets a different signal: `OPENCODE_RESOURCE_ATTRIBUTES` → the OTel `Resource` (producer metadata); `OPENCODE_SPAN_ATTRIBUTES` → spans only; `OPENCODE_METRIC_ATTRIBUTES` → metric data points only. They are independent — a key set in one does not appear in the others.
